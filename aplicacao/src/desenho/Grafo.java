...
public class Grafo {
    ...
    public Grafo(Pane parent, int nVert, Adjacencia lista, boolean topologica, ArrayList<codificacoes.buscaProfundidade.Vertice> conjunto) {

        this.parent = parent;
        this.directed = lista.isTipoGrafo();
        this.topologica = topologica;
        ...
        double largura = FXMLPrincipalController.painelD.getWidth() - 100; //dar uma margem das beiradas
        double altura = FXMLPrincipalController.painelD.getHeight();
        double passo = largura / (quantidade - 1);
        ...
        if (!topologica) {
            ...
            for (codificacoes.buscaProfundidade.Vertice v : conjunto) {
                ...
                for (Vertice d : this.vertex) {

                    if (d.getID() == v.getPosicao()) {

                        d.setLayoutX(50 + passo * cont);
                        d.setLayoutY(altura / 2);
                        cont++;
                        break;
                    }
                }
            ...
            for (int j = 0; j < nVert; j++) {
                ...
                if (lista.verificaAdjacencia(i, j)) {
                    ...
                    if (i == j) {
                        ...
                        a = new Loop(this.getVertice(i), lista.getPeso(i, j), directed, topologica);
                    } else if (lista.verificaAdjacencia(j, i) || topologica) {

                        a = new Arco(this.getVertice(i), this.getVertice(j), lista.getPeso(i, j), directed, topologica);
                    } else {
