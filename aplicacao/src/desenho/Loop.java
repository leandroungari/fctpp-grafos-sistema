...
public class Loop extends Aresta {
    ...
    public void setInicio() {
        ...
        Grafo principal = FXMLPrincipalController.getGrafo();

        if (Desenho.novo != null && Desenho.novo.getTopologica()) {
        ...
        for (Vertice vertice : principal.vertex) {
            centerX += vertice.getLayoutX();
            centerY += vertice.getLayoutY();
        }

        centerX /= principal.vertex.size();
        centerY /= principal.vertex.size();
        ...
        this.forma.setCenterX(meioX - (xAB * -cos - yA * -sen));
        this.forma.setCenterY(meioY - (xAB * -sen + yA * -cos));
